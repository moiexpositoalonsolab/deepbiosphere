...
class CrossEntropyPresenceOnly(nn.Module):
    ...
    def __init__(self, sum=False,class_weights=None,  weighting=False, device='cpu', type='sum'):
        super().__init__()
        self.type = type
...
# for scaled BCE Experiment: gamma_neg=0, gamma_pos=0, clip=0.0, eps=1e-8, disable_torch_grad_focal_loss=False
    def __init__(self, gamma_neg=4, gamma_pos=1, clip=0.05, eps=1e-8, disable_torch_grad_focal_loss=False):
        super().__init__()
        self.gamma_neg = gamma_neg
        self.gamma_pos = gamma_pos
...
# standard is gamma-=4, gamma+=1, clip=0.05. gamma > 0 turns on scaling by predicted prob
# for scaled BCE Experiment: gamma_neg=0, gamma_pos=0, clip=0.0, eps=1e-8, disable_torch_grad_focal_loss=False
    def __init__(self, gamma_neg=0, gamma_pos=0, clip=0.0, eps=1e-8, disable_torch_grad_focal_loss=False):
        super().__init__()
        self.gamma_neg = gamma_neg
        self.gamma_pos = gamma_pos
        self.clip = clip
